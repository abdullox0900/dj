...
export const CalcModal = () => {
  ...
  const { isOpenCalcModal: isOpen } = useAppSelector(main);
  const dispatch = useAppDispatch();

  useOutside(ref, () => dispatch(setOpenCalcModal(false)));
  ...
  const formHandler = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await axios.post("/api/calc", reqData).then();

      dispatch(setOpenCalcModal(false));
