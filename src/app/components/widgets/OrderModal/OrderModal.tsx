...
export const OrderModal: FC<Props> = ({
  ...
  const formHandler = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!tel || !name) return toast.error(t("Fill in all the fields!"))
  ...
  return (
    <div
      className={cn(
        "fixed top-0 bottom-0 left-0 right-0 bg-black/20 ease-linear duration-200 z-50",
        {
    ...
    >
      <div
        ...
        ref={ref}
      >
        <h5 className="text-center font-semibold text-lg md:text-xl lg:text-2xl xl:text-3xl">
          {t("Learn more")}
        </h5>

        <form
          className="flex flex-col gap-2.5 lg:gap-6 mt-5 lg:mt-8"
        ...
        >
          ...
          <Button
            type="submit"
            title={isLoading ? t("Loading") : t("Learn more")}
            disabled={isLoading}
            className="h-[50px]"
          />
        </form>
